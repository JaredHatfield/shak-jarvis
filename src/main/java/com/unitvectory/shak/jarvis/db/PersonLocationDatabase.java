...
public class PersonLocationDatabase extends AbstractDatabase implements
    ...
    public InsertResult insertLocation(PersonLocationPublish publish) {
        ...
        InsertResult historyResult =
                this.insertLocationEvent(person, place, publish.getStatus());
     ...
     */
    private static final String InsertPersonLocationEventQuery =
            "INSERT INTO person_location_event "
    ...
    private InsertResult
            insertLocationEvent(int person, int place, char status) {
        Connection con = null;
        PreparedStatement stmt = null;
        try {
            con = this.getConnection();
     ...
     */
    private static final String InsertPersonLocationRecentQuery =
            "INSERT INTO person_location_recent "
                    + "(person, place, status, occurred) "
                    + "VALUES (?,?,?,NOW()) "
     ...
     */
    private InsertResult
            insertLocationRecent(int person, int place, char status) {
        Connection con = null;
        PreparedStatement stmt = null;
        try {
            con = this.getConnection();
     ...
     * the person id query
     */
    private static final String PersonIdQuery =
            "SELECT id FROM person WHERE token = ? ";
     ...
     */
    private static final String PersonPlaceIdQuery =
            "SELECT id FROM person_place WHERE name = ? ";
